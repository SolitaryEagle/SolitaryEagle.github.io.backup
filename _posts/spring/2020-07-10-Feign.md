...
### Default
默认的客户端, 使用 HttpURLConnection 发送请求

### Proxied
基于 Default, 添加了 Proxy-Authorization 的功能

### LoadBalancerFeignClient
Ribbon 的负载均衡客户端

### FeignBlockingLoadBalancerClient
spring cloud loadbalancer 负载均衡客户端

## Contract
定义哪些注解和值在接口上是有效的

### BaseContract
Contract 的基类, 获取 MethodMetadata 的通用模板
...
处理 HystrixCommand, Observable, Single
...
### SpringEncoder
与 SpringMVC 结合的 Encoder

### PageableSpringEncoder
支持分页查询 Pageable 的 Encoder

## Decoder
![](https://raw.githubusercontent.com/SolitaryEagle/SolitaryEagle.github.io/master/images/feign.Decoder.png)
...
### StringDecoder
直接将 Response.Body 转换为 String, 适用于 Feign.Client 的接口返回值类型为 String.

### Default
如果 response.status() == 404 || response.status() == 204 返回 Util.emptyValueOf(type).

如果 byte[].class.equals(type) 返回 Util.toByteArray(response.body().asInputStream()).

其他情况使用 StringDecoder

### StreamDecoder
将 Response 解码为 Stream<T>

### ResponseMappingDecoder
在解码之前先用 ResponseMapper 处理一下 Response

### OptionalDecoder
将 Response 解码为 Optional

### DefaultGzipDecoder
如果 Response 被以 gzip 形式压缩, 先将其解压, 再使用 SpringDecoder 解码

### ResponseEntityDecoder
解码器通过组合增加了 SpringMVC 对任何其他解码器响应的兼容性。

### SpringDecoder
与 SpringMVC 结合的 Decoder
